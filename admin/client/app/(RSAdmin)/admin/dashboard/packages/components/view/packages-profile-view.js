...
import PassengerProfileCover from "../packages-profile-cover";
import { LoadingScreen } from "@/app/(RSAdmin)/admin/common/loading-screen";
import axios from "axios";
import { endpoints } from "@/app/(RSAdmin)/admin/utils/axios";

export default function PackagesProfileView({ id }) {
  const [passenger, setPassenger] = useState({});
  const [loading, setLoading] = useState(true);

  const fetch = async () => {
    setLoading(true);
    const { data, status } = await axios.get(endpoints.packages.byId(id))
    if (status === 200) {
      setPassenger(data);
      setLoading(false);
